...
function MonthDay({ day, isToday, month, summary }: { day: number; isToday: boolean; month: MonthValue; summary?: MonthlyReservationSummary }) {
  ...
  const blocked = summary?.blocked ?? false;
  return <Link aria-label={`${day}: ${reservations} ${reservations === 1 ? "reserva" : "reservas"}, ${people} ${people === 1 ? "pessoa" : "pessoas"}${blocked ? ", dia bloqueado" : ""}`} className={`focus-ring flex h-[72px] min-w-0 flex-col overflow-hidden rounded-[9px] border p-1.5 lg:h-[110px] lg:rounded-[14px] lg:p-3 ${blocked ? "border-[#d78379] bg-[#fff1f1]" : isToday ? "border-[1.5px] border-[#a64f43] bg-[#f6ede7]" : "border-[#e3ddd4] bg-[#faf9f6] lg:bg-white"}`} href={`/?date=${value}`}><span className={`flex items-center justify-between text-[11px] font-semibold lg:text-[13px] ${isToday ? "text-[#a64f43]" : ""}`}><span>{day}<span className="sr-only">{isToday ? " Hoje" : ""}</span></span>{blocked ? <span aria-hidden="true">🔒</span> : null}</span><span className="mt-auto truncate text-[9px] font-semibold text-[#3f7450] lg:mt-2 lg:text-xs lg:text-[#202421]">{reservations} <span className="lg:hidden">res.</span><span className="hidden lg:inline">{reservations === 1 ? "reserva" : "reservas"}</span></span><span className="truncate text-[9px] text-[#727870] lg:mt-1 lg:text-[11px]">{people} <span className="lg:hidden">pess.</span><span className="hidden lg:inline">{people === 1 ? "pessoa" : "pessoas"}</span></span>{blocked ? <span className="hidden text-[10px] font-semibold text-[#a64f43] lg:block">🔒 Bloqueado</span> : null}</Link>;
}
