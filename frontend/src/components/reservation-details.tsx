  ...
  return (
    <AppShell onLogout={handleLogout} user={user}>
      <ReservationDetails
        ...
        onReservationChanged={(updatedReservation, feedback) => {
...
function ReservationDetails({
  ...
  agendaDate?: string;
  onReservationChanged: (reservation: Reservation, feedback: string) => void;
  onNotFound: () => void;
  ...
  return (
    <div className="px-4 pb-10 pt-4 lg:px-9 lg:py-8">
      ...
      {actionFeedback || reservationUpdated ? <p className="mt-3 rounded-xl border border-[#b8d3bf] bg-[#e9f3eb] px-4 py-3 text-sm font-medium text-[#2f6240]" role="status">{actionFeedback || "Reserva atualizada com sucesso."}</p> : null}
...
function FutureActions({
  ...
  function openDialog(nextAction: Action) {
    setActionError("");
  ...
  async function handleAction() {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        setActionError("O estado desta reserva mudou. Atualize os dados antes de tentar novamente.");
        return;
      }
      if (!response.ok) {
