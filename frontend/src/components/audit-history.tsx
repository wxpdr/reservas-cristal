...
import { getAuditEvents, getErrorMessage, getUsers, type AuditEvent, type AuthenticatedUser } from "@/lib/api";
...
export function AuditHistory() {
  ...
  const loadEvents = useCallback(async () => {
    try {
      const response = await getAuditEvents({ date, userId, action });
  ...
  function clearFilters() { setDate(""); setUserId(""); setAction(""); }
  return <div className="px-4 pb-10 pt-4 lg:px-9 lg:py-8"><header className="min-h-[72px]"><h1 className="text-2xl font-semibold lg:text-[28px]">Histórico<span className="hidden lg:inline"> de alterações</span></h1><p className="mt-1 text-xs text-[#727870] lg:text-[13px]">Consulte as principais ações realizadas nas reservas.</p></header><section className="mt-1 grid gap-3 rounded-[13px] border border-[#e3ddd4] bg-white p-3 lg:mt-4 lg:grid-cols-[170px_260px_260px_1fr] lg:items-end"><label className="text-[11px] font-semibold text-[#727870]">Data<input className="form-control mt-1" onChange={(event) => setDate(event.target.value)} type="date" value={date} /></label><label className="text-[11px] font-semibold text-[#727870]">Usuário<select className="form-control mt-1" onChange={(event) => setUserId(event.target.value)} value={userId}><option value="">Todos os usuários</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label className="text-[11px] font-semibold text-[#727870]">Ação<select className="form-control mt-1" onChange={(event) => setAction(event.target.value)} value={action}><option value="">Todas as ações</option>{actions.map((item) => <option key={item} value={item}>{actionLabels[item]}</option>)}</select></label><button className="secondary-button h-10 justify-self-end px-4 text-xs" onClick={clearFilters} type="button">Limpar filtros</button></section><div className="mt-5 flex items-center justify-between"><h2 className="text-lg font-semibold">Eventos {date ? "do dia" : "encontrados"}</h2><p className="text-xs text-[#727870]">{events.length} {events.length === 1 ? "evento encontrado" : "eventos encontrados"}</p></div>{error ? <p className="mt-3 rounded-xl bg-[#fff1f1] p-4 text-sm text-[#8b3030]" role="alert">{error}</p> : null}<section aria-busy={loading} className="mt-3 space-y-2.5">{loading ? [1,2,3].map((item) => <div className="h-28 animate-pulse rounded-[14px] bg-white lg:h-20" key={item} />) : events.length ? events.map((event) => <AuditRow event={event} key={event.id} />) : <div className="rounded-[14px] border border-[#e3ddd4] bg-white px-5 py-10 text-center text-sm text-[#727870]">Nenhum evento encontrado para os filtros selecionados.</div>}</section><p className="mt-4 rounded-xl bg-[#f0ece6] px-4 py-4 text-xs text-[#727870]">Esta área é administrativa e serve para rastreabilidade. Ela não aparece na operação diária dos atendentes.</p></div>;
}
